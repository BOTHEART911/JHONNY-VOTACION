...
function viewInstalar() {
  ...
  const cont = () => { sessionStorage.setItem(nsKey('continuedWeb'), '1'); go('escaner'); };
...
function viewEscaner() {
  app.innerHTML = `
    <div class="appbar">
      ...
      <div class="who"><b>Registro de Votación</b><span>${esc(MV.candidato)}</span></div>
...
async function registrar(p) {
  ...
  try {
    ...
    /* 25/07: solo vota quien tenga MUNICIPIO = FLANDES en la base. */
    if (d.noFlandes) {
      sonar('err'); vibrar([120, 60, 120]);
      resultado('err', 'NO VOTA EN ' + MV.municipio.toUpperCase(), nombreYMunicipio(d.persona, d.municipio));
      return d;
    }
    if (d.yaVoto) {
...
function aplicarLote(items) {
  let nOk = 0, nYa = 0, nMal = 0, nFuera = 0, ultimo = '';   // 25/07: nFuera = no vota en Flandes
  items.forEach(it => {
    ...
    } else if (it.estado === 'noFlandes') {
      nFuera++;
      colaLote().marcar(id, 'malo');
      marcarItemLote(id, 'malo', nombre, 'No vota en ' + MV.municipio + (extra ? ' · ' + extra : ''));
  ...
  if (nOk) {
    ...
    resultado('ok', nOk === 1 ? 'VOTO REGISTRADO' : (nOk + ' VOTOS REGISTRADOS'),
      [ultimo, (nYa ? nYa + ' ya estaba(n)' : ''), (nFuera ? nFuera + ' no vota(n) en ' + MV.municipio : ''), (nMal ? nMal + ' sin base' : '')].filter(Boolean).join(' · '));
  } else if (nYa) {
    sonar('info'); vibrar(80);
    resultado('warn', nYa === 1 ? 'YA ESTABA REGISTRADA' : (nYa + ' YA ESTABAN REGISTRADAS'),
      [(nFuera ? nFuera + ' no vota(n) en ' + MV.municipio : ''), (nMal ? nMal + ' no está(n) en la base de datos' : '')].filter(Boolean).join(' · ') || 'No se agregó ninguna fila nueva.');
  } else if (nFuera) {
    sonar('err'); vibrar(200);
    resultado('err', nFuera === 1 ? 'NO VOTA EN ' + MV.municipio.toUpperCase() : (nFuera + ' NO VOTAN EN ' + MV.municipio.toUpperCase()),
      nMal ? (nMal + ' no está(n) en la base de datos') : 'No se registró ninguno.');
  } else if (nMal) {
    sonar('err'); vibrar(200);
...
/* 25/07: nombre + lo que dice la columna MUNICIPIO, para el aviso de
   "No vota en Flandes". Si viene vacía se dice tal cual. */
function nombreYMunicipio(per, municipio) {
  const n = String((per && per.nombre) || '').trim();
  const m = String(municipio || (per && per.municipio) || '').trim();
  return [n, m ? 'Municipio: ' + m : 'Sin municipio registrado'].filter(Boolean).join(' — ');
}
