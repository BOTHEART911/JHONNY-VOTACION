...
async function registrar(p) {
  ...
  try {
    ...
    if (d.noFlandes) {
      sonar('err'); vibrar([120, 60, 120]);
      resultado('err', 'NO VOTA EN FLANDES', nombreYMunicipio(d.persona, d.municipio));
      return d;
    }
    if (d.yaVoto) {
...
function aplicarLote(items) {
  ...
  items.forEach(it => {
    ...
    if (it.estado === 'ok') {
      ...
      marcarItemLote(id, 'malo', nombre, 'No vota en Flandes' + (extra ? ' · ' + extra : ''));
  ...
  if (nOk) {
    ...
    resultado('ok', nOk === 1 ? 'VOTO REGISTRADO' : (nOk + ' VOTOS REGISTRADOS'),
      [ultimo, (nYa ? nYa + ' ya estaba(n)' : ''), (nFuera ? nFuera + ' no vota(n) en Flandes' : ''), (nMal ? nMal + ' sin base' : '')].filter(Boolean).join(' · '));
  } else if (nYa) {
    sonar('info'); vibrar(80);
    resultado('warn', nYa === 1 ? 'YA ESTABA REGISTRADA' : (nYa + ' YA ESTABAN REGISTRADAS'),
      [(nFuera ? nFuera + ' no vota(n) en Flandes' : ''), (nMal ? nMal + ' no está(n) en la base de datos' : '')].filter(Boolean).join(' · ') || 'No se agregó ninguna fila nueva.');
  } else if (nFuera) {
    sonar('err'); vibrar(200);
    resultado('err', nFuera === 1 ? 'NO VOTA EN FLANDES' : (nFuera + ' NO VOTAN EN FLANDES'),
